import{db,roomPath,roomId,ref,onValue,get,set,update,serverTimestamp,ensureRoom,normalize,GROUP_COLORS,drawBase,seatEl,sortSeats,makeGuestCode,qrPayload,createCloudBackup,setupViewport,defaultRoom,mergeRoomDefaults,esc,logActivity,saveUndoSnapshot,undoLastChange}from'./common.js';
...
function guardPlan(){if(!isLocked())return true;alert('SEATING PLAN IS LOCKED. Unlock the plan before editing.');return false}
function render(){
...
function renderPanel(){
  ...
  seatTitle.textContent='Seat '+s.id;statusInput.value=s.status;guestInput.value=s.guestName||'';groupInput.value=s.groupName||'';groupIdInput.value=s.groupId||'';colorInput.value=s.groupColor||GROUP_COLORS[0].value;souvenirInput.value=s.souvenirEligible?'yes':'no';conditionsInput.value=s.conditions||'';notesInput.value=s.notes||'';
  seatPanel.querySelectorAll('input,textarea,select,button').forEach(el=>el.disabled=isLocked());
}
function renderControls(){
  tableControls.innerHTML='';Object.values(state.tables).sort((a,b)=>a.id.localeCompare(b.id)).forEach(t=>{const d=document.createElement('div');d.className='table-control';d.innerHTML=`<strong>${t.id}</strong><button>+ Row</button><button>− Row</button>`;d.children[1].disabled=isLocked();d.children[2].disabled=isLocked();d.children[1].onclick=()=>addRow(t.id);d.children[2].onclick=()=>removeRow(t.id);tableControls.appendChild(d)});
  [moveGuestMode,moveGroupMode].forEach(b=>b.disabled=isLocked());
}
function renderPlanSafety(){
  ...
  if(undoBtn)undoBtn.disabled=isLocked();
}
...
async function addRow(id){
  if(!guardPlan())return;const t=state.tables[id];if(!t)return alert('Table '+id+' is not ready. Please refresh.');
...
async function removeRow(id){
  if(!guardPlan())return;const t=state.tables[id],r=t.rows-1,targets=Object.values(state.seats).filter(s=>s.tableId===id&&s.rowIndex===r);if(targets.some(s=>s.guestName||s.confirmed||s.checkedIn))return alert('Last row is in use.');if(!confirm('Remove last row?'))return;
...
selectMode.onclick=()=>mode('select');moveGuestMode.onclick=()=>{if(guardPlan())mode('guest')};moveGroupMode.onclick=()=>{if(guardPlan())mode('group')};
async function clickSeat(s){
  if(state.mode==='select'){state.selected=s.id;return render()}
  if(!guardPlan())return mode('select');
...
function formData(s){return{...s,status:statusInput.value,guestName:guestInput.value.trim(),groupName:groupInput.value.trim(),groupId:groupIdInput.value.trim(),groupColor:colorInput.value,souvenirEligible:souvenirInput.value==='yes',conditions:conditionsInput.value.trim(),notes:notesInput.value.trim(),updatedAt:Date.now()}}
saveBtn.onclick=async()=>{if(!guardPlan())return;const s=state.seats[state.selected];if(s){await saveUndoSnapshot(`Edit seat ${s.id}`);const next=formData(s);await set(ref(db,roomPath(`seats/${s.id}`)),next);await logActivity('Seat updated',`${s.id} ${next.guestName||''}`)}};
confirmBtn.onclick=async()=>{if(!guardPlan())return;let s=state.seats[state.selected];if(!s||!guestInput.value.trim())return alert('Enter guest name.');await saveUndoSnapshot(`Confirm seat ${s.id}`);s={...formData(s),status:'reserved',confirmed:true,guestCode:s.guestCode||makeGuestCode()};await set(ref(db,roomPath(`seats/${s.id}`)),s);await logActivity('Seat confirmed',`${s.id} ${s.guestName}`);qrcode.innerHTML='';new QRCode(qrcode,{text:qrPayload(s),width:260,height:260,correctLevel:QRCode.CorrectLevel.H});qrInfo.innerHTML=`<strong>${esc(s.id)}</strong><br>${esc(s.guestName)}`;qrModal.classList.add('open');downloadQr.onclick=()=>{const img=qrcode.querySelector('img')||qrcode.querySelector('canvas');const a=document.createElement('a');a.download=`${s.id}-${s.guestName||'guest'}.png`;a.href=img.tagName==='CANVAS'?img.toDataURL('image/png'):img.src;a.click()}};
clearBtn.onclick=async()=>{if(!guardPlan())return;const s=state.seats[state.selected];if(s){await saveUndoSnapshot(`Clear seat ${s.id}`);await set(ref(db,roomPath(`seats/${s.id}`)),{...s,status:'available',guestName:'',groupName:'',groupId:'',conditions:'',notes:'',souvenirEligible:false,confirmed:false,guestCode:'',checkedIn:false,checkedInAt:null});await logActivity('Reservation cleared',s.id)}};
...
undoBtn.onclick=async()=>{if(!guardPlan())return;if(!confirm('Undo the last Admin change?'))return;undoBtn.disabled=true;try{const ok=await undoLastChange();if(!ok)alert('No undo snapshot is available.')}finally{undoBtn.disabled=false}};
