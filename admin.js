...
async function persistSeat(s,extra={}){if(!s)return null;const patch={...formPatch(),...extra};await update(ref(db,roomPath(`seats/${s.id}`)),patch);return{...s,...patch}}
saveBtn.onclick=async()=>{if(!guardPlan())return;const s=state.seats[state.selected];if(!s)return;try{saveBtn.disabled=true;await saveUndoSnapshot(`Edit seat ${s.id}`);const next=await persistSeat(s);await logActivity('Seat updated',`${s.id} ${next.guestName||''}${next.souvenirEligible?' · Gift redemption':''}`)}catch(err){console.error(err);alert('Could not save this seat: '+err.message)}finally{saveBtn.disabled=false}};
souvenirInput.addEventListener('change',async()=>{if(!state.selected||isLocked())return;const s=state.seats[state.selected];if(!s)return;try{await update(ref(db,roomPath(`seats/${s.id}`)),{souvenirEligible:souvenirInput.value==='yes',updatedAt:Date.now()});await logActivity('Gift redemption changed',`${s.id}: ${souvenirInput.value==='yes'?'YES':'NO'}`)}catch(err){console.error(err);alert('Could not update Gift redemption: '+err.message)}});
confirmBtn.onclick=async()=>{if(!guardPlan())return;let s=state.seats[state.selected];if(!s||!guestInput.value.trim())return alert('Enter guest name.');try{confirmBtn.disabled=true;await saveUndoSnapshot(`Confirm seat ${s.id}`);s=await persistSeat(s,{status:'reserved',confirmed:true,guestCode:s.guestCode||makeGuestCode()});await logActivity('Seat confirmed',`${s.id} ${s.guestName}${s.souvenirEligible?' · Gift redemption':''}`);qrcode.innerHTML='';new QRCode(qrcode,{text:qrPayload(s),width:260,height:260,correctLevel:QRCode.CorrectLevel.H});qrInfo.innerHTML=`<strong>${esc(s.id)}</strong><br>${esc(s.guestName)}`;qrModal.classList.add('open');downloadQr.onclick=()=>{const img=qrcode.querySelector('img')||qrcode.querySelector('canvas');const a=document.createElement('a');a.download=`${s.id}-${s.guestName||'guest'}.png`;a.href=img.tagName==='CANVAS'?img.toDataURL('image/png'):img.src;a.click()}}catch(err){console.error(err);alert('Could not confirm this guest: '+err.message)}finally{confirmBtn.disabled=false}};
