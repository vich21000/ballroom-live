function doPost(e) {
  try {
    ...
    sh.clearContents();
    const headers = ['Seat','Guest','Group','Group ID','Status','Confirmed','Checked In','Checked In At','Conditions','Notes'];
    const values = (payload.rows || []).map(s => [s.id,s.guestName,s.groupName,s.groupId,s.status,!!s.confirmed,!!s.checkedIn,s.checkedInAt ? new Date(s.checkedInAt) : '',s.conditions,s.notes]);
