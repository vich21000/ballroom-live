...
export const TABLE_X={A:240,B:620,C:1000,D:1560,E:1940,F:2320};export const TABLE_W=90,SEAT_W=82,GAP=32,ROW_GAP=62,STAGE_CENTER=1325,AISLE_LEFT=1210,AISLE_W=230;
...
export function setupViewport(which){const viewport=document.getElementById(which+'Viewport'),world=document.getElementById(which+'World');let z=1,drag=false,sx=0,sy=0,sl=0,st=0;const center=()=>{viewport.scrollLeft=Math.max(0,(2640*z-viewport.clientWidth)/2);viewport.scrollTop=Math.max(0,(1180*z-viewport.clientHeight)/2)};const apply=()=>{world.style.transform=`scale(${z})`;center()};const fit=()=>{z=Math.max(.32,Math.min(2.2,Math.min((viewport.clientWidth-24)/2640,(viewport.clientHeight-24)/1180)));apply()};document.querySelectorAll(`[data-zoom="${which}"]`).forEach(b=>b.onclick=()=>{const a=b.dataset.action;if(a==='in')z=Math.min(2.4,z+.1);if(a==='out')z=Math.max(.32,z-.1);if(a==='reset')z=1;if(a==='fit')return fit();apply()});viewport.onmousedown=e=>{if(e.target.closest('button,input'))return;drag=true;viewport.classList.add('panning');sx=e.pageX;sy=e.pageY;sl=viewport.scrollLeft;st=viewport.scrollTop};window.addEventListener('mouseup',()=>{drag=false;viewport.classList.remove('panning')});viewport.onmousemove=e=>{if(!drag)return;viewport.scrollLeft=sl-(e.pageX-sx);viewport.scrollTop=st-(e.pageY-sy)};window.addEventListener('resize',fit);requestAnimationFrame(fit);return{fit,apply,center}}
