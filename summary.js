import{db,roomPath,roomId,ref,onValue,get,set,ensureRoom,normalize,sortSeats,esc,createCloudBackup,defaultRoom,isGiftEligible}from'./common.js?v=20260921-fixv2';import{APPS_SCRIPT_WEB_APP_URL,GOOGLE_SHEET_ID}from'./google-sheet-config.js';await ensureRoom();let seats={};onValue(ref(db,roomPath('seats')),s=>{seats=normalize(s.val());render()});function rows(){return Object.values(seats).sort(sortSeats)}function render(){const q=summarySearch.value.trim().toLowerCase(),a=rows();summaryBody.innerHTML=a.filter(s=>!q||[s.id,s.guestName,s.groupName,s.status,isGiftEligible(s)?'gift':'',s.conditions,s.notes].join(' ').toLowerCase().includes(q)).map(s=>`<tr><td>${esc(s.id)}</td><td>${esc(s.guestName)}</td><td>${esc(s.groupName)}</td><td>${esc(s.status)}</td><td>${s.confirmed?'Yes':'No'}</td><td>${s.checkedIn?'Yes':'No'}</td><td>${isGiftEligible(s)?'Yes':'No'}</td><td>${esc(s.conditions)}</td><td>${esc(s.notes)}</td></tr>`).join('');sumTotal.textContent=a.length;sumAssigned.textContent=a.filter(s=>s.guestName||s.groupName).length;sumChecked.textContent=a.filter(s=>s.checkedIn).length;sumNotArrived.textContent=a.filter(s=>(s.guestName||s.groupName)&&!s.checkedIn).length;sumGift.textContent=a.filter(isGiftEligible).length}summarySearch.oninput=render;function csv(){const h=['Seat','Guest','Group','Group ID','Status','Confirmed','Checked In','Gift Redemption','Conditions','Notes'];const body=rows().map(s=>[s.id,s.guestName,s.groupName,s.groupId,s.status,s.confirmed,s.checkedIn,isGiftEligible(s)?'Yes':'No',s.conditions,s.notes].map(v=>'"'+String(v??'').replaceAll('"','""')+'"').join(','));return[h.join(','),...body].join('\n')}csvBtn.onclick=()=>{const b=new Blob([csv()],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`ballroom-${roomId}.csv`;a.click()};sheetBtn.onclick=async()=>{
